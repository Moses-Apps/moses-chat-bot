...
- **Bot connection (moses-chat-bot-qcq)**: a tenant admin connects a bot entirely in-app via the "Connect Telegram" wizard (`frontend/src/pages/ConnectTelegram.tsx`, route `/settings/telegram`). Telegram bots can only be created by a human through @BotFather — the wizard does NOT emulate BotFather; it hands the admin off via a `https://t.me/botfather` deep link + copy-paste `/newbot` commands, then takes the resulting token back through a paste field. After the paste there is nothing else for the admin to do: long-polling (below) means no webhook URL, no public-access change, and no HTTPS/tunnel setup.
- **Token at rest**: the bot token is stored encrypted per-tenant in the `telegram_bot_config` table (one row per tenant), sealed under the existing AES-256-GCM crypto envelope (`internal/service/crypto`, tenant-keyed DEK) — never plaintext. `encryption_key_id` records the master-key version so it survives rotation. The `TELEGRAM_BOT_TOKEN` env var remains a **bootstrap/legacy fallback**: it is used by `botconfig.LoadAtStartup` only when no `telegram_bot_config` row exists.
- **Inbound mode — long-polling by default (moses-chat-bot-9so)**: the bot receives messages by long-polling the Telegram `getUpdates` method, NOT by webhook. A webhook forces the deployed app to be publicly reachable, which conflicts with the Moses per-app auth gate; long-polling is purely outbound, so it works behind the gate, on localhost, with no tunnel and no public exposure. The poll loop (`provider/telegram/pollloop.go`) `deleteWebhook`s first (Telegram returns 409 from `getUpdates` while a webhook is active), then calls `getUpdates` with a ~30s long-poll `timeout`, `allowed_updates=["message"]`, and an `offset` advanced to `max(update_id)+1`. The offset is held **in memory only** — on restart the loop re-fetches from offset 0, but the relay dedups by `provider_message_id`, so at-most-once delivery still holds. The loop respects context cancellation for a clean shutdown, backs off transient errors (1s→30s cap), and stops on a revoked token (`ErrUnauthorized`). `getUpdates` is single-consumer (Telegram hands each update to exactly one caller), so exactly one poll loop may run per bot token — the deployment keeps `replicas: 1`; scaling out would require webhook mode or a leader-elected single poller (a future bead).
- **Inbound mode — webhook is opt-in**: webhook delivery is enabled only when the `BOT_WEBHOOK_PUBLIC_URL` env var is set. In that mode `botconfig.Connect` registers a webhook at that base URL + `/api/v1/providers/telegram/webhook` (instead of starting a poll loop) and generates a random `webhook_secret` passed as `setWebhook`'s `secret_token`; Telegram echoes it in the `X-Telegram-Bot-Api-Secret-Token` header, which the live adapter verifies per request. The webhook route is mounted unconditionally regardless of mode; in long-polling mode it simply never receives traffic (Telegram is never told the URL) and returns 503 when no bot is connected.
- **Endpoints** (`internal/handler/telegram_config.go`): `GET /api/v1/provider/telegram/info` (tenant read, any member — backs the honest LinkNew states); `POST` / `DELETE /api/v1/provider/telegram/connect` (tenant-admin gated via `middleware.RequireTenantAdmin`, which reads the role from the platform `/auth/me` membership). `Connect` validates the token with `getMe`, encrypts + upserts, calls `setMyCommands`, registers the in-process adapter, and — in the default mode — starts the poll loop; all at runtime, no redeploy.
- **Dynamic adapter** (`internal/service/botconfig`): on startup the adapter is hydrated from the DB; on connect it is swapped in the shared provider registry. `botconfig` also owns the poll-loop lifecycle — it starts the loop on `Connect`/`LoadAtStartup` (default mode), stops it on `Disconnect`, and restarts it when the adapter is swapped. Both the webhook path and the poll loop funnel through the same `updateToInbound` Update→`InboundMessage` conversion, so the relay is entirely mode-agnostic.
