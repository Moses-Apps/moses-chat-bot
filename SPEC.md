...
- **Bot connection (moses-chat-bot-qcq)**: a tenant admin connects a bot entirely in-app via the "Connect Telegram" wizard (`frontend/src/pages/ConnectTelegram.tsx`, route `/settings/telegram`). Telegram bots can only be created by a human through @BotFather — the wizard does NOT emulate BotFather; it hands the admin off via a `https://t.me/botfather` deep link + copy-paste `/newbot` commands, then takes the resulting token back through a paste field.
...
- **Endpoints** (`internal/handler/telegram_config.go`): `GET /api/v1/provider/telegram/info` (tenant read, any member — backs the honest LinkNew states); `POST` / `DELETE /api/v1/provider/telegram/connect` (tenant-admin gated via `middleware.RequireTenantAdmin`, which reads the role from the platform `/auth/me` membership). `Connect` validates the token with `getMe`, encrypts + upserts, calls `setWebhook` + `setMyCommands`, and registers the in-process adapter at runtime — no redeploy.
- **Dynamic adapter** (`internal/service/botconfig`): on startup the adapter is hydrated from the DB; on connect it is swapped in the shared provider registry. The webhook route (`/api/v1/providers/telegram/webhook`) is mounted unconditionally and resolves the live adapter per request (503 when none is connected), so connect/disconnect take effect immediately.
- Webhook URL: derived from the connect request's `Host` header + `MOSES_BASE_PATH` + `/api/v1/providers/telegram/webhook`, so the bot registers the URL it is actually reachable on.
- Signature verification: Telegram doesn't sign webhooks; instead `botconfig.Connect` generates a random `webhook_secret`, persists it on the `telegram_bot_config` row, and passes it as the `secret_token` on `setWebhook`. Telegram echoes it as the `X-Telegram-Bot-Api-Secret-Token` header; the live adapter (built from the stored secret) verifies it per request.
